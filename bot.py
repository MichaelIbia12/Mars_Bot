...
class Mars():
    ...
    def response_engine(self, txt: str):
        ...
        if prediction == 404:
            ...
            if 'recommendations' in personal_split:
                meaning = self.recomendation(txt)
                response += " " + meaning
    ...
    def recomendation(self, text: str):
        for i in self.vocabulary["recommendations"][0]:
            if  i in text.lower().split():
                unknown, subject = text.split(i.lower())          
                print(subject)
